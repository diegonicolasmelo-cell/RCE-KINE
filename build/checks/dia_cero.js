...
// (ago-2026).
//
// REPORTE DE DIEGO (4-ago): Rosa ingresa y se intuba el MISMO día. La grilla
...
(async () => {
  ...
  const filas = await p.evaluate(() => {
    ...
    DB = [
      // 1 · Rosa: ingresa y se intuba HOY ⇒ Día 0 de estadía y de VM
      { ID_CAMA: '1', OCUPADA: true, NOMBRE: 'Rosa Quiroga', SEXO: 'F', EDAD: 70,
        FECHA_INGRESO: '2026-08-04', FECHA_INICIO_SOPORTE: '2026-08-04', VIA_AEREA: 'TOT', SOPORTE: 'VM' },
      // 2 · Paciente ventilado hace días
      { ID_CAMA: '2', OCUPADA: true, NOMBRE: 'Maria Inelda', SEXO: 'F', EDAD: 73,
    ...
    EVOS_DIA = [
      // Rosa evolucionada: el servidor selló DIAS_VM = 0 (día 0), soporte VM
      { ID_CAMA: '1', TURNO_KEY: '2026-08-04-Dia', DIA_ESTADIA: 0, DIAS_VM: 0, DIAS_VNI: 0,
        VENT_SOPORTE: 'VM', VENT_SOPORTE_FINAL: 'VM', PAC_NOMBRE: 'Rosa Quiroga' },
      { ID_CAMA: '2', TURNO_KEY: '2026-08-04-Dia', DIA_ESTADIA: 6, DIAS_VM: 6, DIAS_VNI: 0,
        VENT_SOPORTE: 'VM', VENT_SOPORTE_FINAL: 'VM', PAC_NOMBRE: 'Maria Inelda' },
  ...
  const rosa = filas.find(f => /Rosa/.test(f[1] || ''));
  const maria = filas.find(f => /Maria/.test(f[1] || ''));
  const sinSop = filas.find(f => /Sin Soporte/.test(f[1] || ''));

  eq('Rosa · estadía Día 0 se ve como «0»', rosa && rosa[4], '0');
  eq('Rosa · VM Día 0 se ve como «0», NO como raya', rosa && rosa[5], '0');
  eq('María · sus 6 días de VM siguen intactos', maria && maria[5], '6');
